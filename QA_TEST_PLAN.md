...
*   **Ca kiểm thử:**
    ...
    *   *TC_REG_001:* Đăng ký trùng tên đăng nhập/SĐT -> ❌ **FAILED (CRITICAL)** (Blocker: Thiếu trường OTP).
    *   *TC_REG_002:* Đăng ký thành công -> ❌ **FAILED (CRITICAL)** (Blocker: Thiếu trường OTP gây lỗi 500).
...
*   **Ca kiểm thử:**
    *   *TC_FP_EMPTY:* Bỏ trống ô SĐT/Email -> ✅ **PASSED** (Hiển thị cảnh báo "Vui lòng nhập số điện thoại hoặc email liên hệ").
    *   *TC_FP_001:* Gửi yêu cầu hợp lệ -> ✅ **PASSED** (Hiển thị popup "Yêu cầu thành công! Vui lòng kiểm tra điện thoại...").
    *   *TC_FP_INVALID:* Nhập sai định dạng (vd: "abc") -> ❌ **FAILED** (Client submits directly, server returns "Chức năng quên mật khẩu chưa sẵn sàng").
...
*   **Ca kiểm thử:**
    ...
    *   *TC_POS_002:* Hoàn tất đơn hàng thanh toán Tiền mặt -> ❌ **FAILED** (Trigger "Insufficient stock" error because stock take is silently failing).
...
*   **Ca kiểm thử:**
    *   *TC_STK_001:* Cân đối kho khi có lệch thừa/thiếu -> ❌ **FAILED (Silent Failure)**. Form redirect back to list but actual stock remains `0`.
...
- **Trạng thái:** ✅ **FIXED LOCALLY / STILL FAILING ON VERCEL** (Đã fix local qua `api_client.dart` nhưng bản build trên Vercel chưa được cập nhật).
...
- **Trạng thái:** ❌ **STILL FAILING** (Chưa có validation, nhập "abc" vẫn bị lọt qua).
...
- **Trạng thái:** ❌ **STILL FAILING** (Blocker: Frontend không có trường nhập OTP khiến luồng Đăng ký bị chặn hoàn toàn).

#### 4. [FEATURE MISSING - ONBOARDING] Chưa tích hợp bản đồ (Google Maps) khi nhập địa chỉ
- **Mức độ:** P2 (Cải thiện UX & Data)
- **Môi trường:** Đang rà soát chức năng
- **Mô tả chi tiết:** Ở màn hình Onboarding (Thiết lập thông tin cửa hàng) và các phần nhập liệu khác, trường nhập "Địa chỉ kinh doanh" hiện tại mới chỉ là ô nhập văn bản (text input) thuần túy. Yêu cầu tích hợp API bản đồ Google Maps (gg map) để hỗ trợ tìm kiếm nhanh, ghim vị trí tọa độ (Lat/Lng) và tự động điền địa chỉ chuẩn xác.
- **Trạng thái:** Cần bổ sung vào tính năng Onboarding và hồ sơ cửa hàng.
...
- **Trạng thái:** ✅ **FIXED LOCALLY / STILL FAILING ON VERCEL** (Đã fix local bằng `BotToast` nhưng bản build trên Vercel chưa được cập nhật).
...
- **Trạng thái:** ✅ **FIXED!** (Client-side validation now successfully catches empty notes and blocks the `POST /api/finance/daily-closings` request).
...
- **Trạng thái:** ✅ **FIXED!** (Semantics tree now properly exposes "Cảnh báo Nghĩa vụ Thuế...").
...
- **Trạng thái:** ❌ **STILL FAILING** (QA Agent xác nhận lỗi vẫn xảy ra trên Production. localStorage không hydrate được token).
...
#### 21. [BUG - ROUTER] Lỗi điều hướng 404 (GoException) khi truy cập đường dẫn /employees
- **Mức độ:** P1 (Lỗi hệ thống/Routing chặn luồng)
- **Môi trường:** Deployed (Vercel Production) / App Web
- **Mô tả chi tiết:** Khi truy cập chức năng Quản lý Nhân viên (có thể qua menu hoặc link), ứng dụng crash màn hình trắng và văng lỗi `Page Not Found - GoException: no routes for location: /employees`. Lỗi này xảy ra do định tuyến bị sai lệch (ví dụ link là `/employees` nhưng GoRouter lại đang cấu hình là `/staff`).
- **Kết quả kỳ vọng:** Kiểm tra lại liên kết menu hoặc sửa cấu hình `app_router.dart` để đảm bảo route `/employees` trỏ đúng về `StaffManagementScreen`.
- **Trạng thái:** ✅ **FIXED** (Navigating to `/employees` gracefully redirects to Auth instead of crashing the router).

#### 22. [BUG - SYSTEM] Dashboard văng lỗi và trắng màn hình nếu Shop Array bị rỗng
- **Mức độ:** P0 (Chặn đứng hệ thống - Crash)
- **Môi trường:** Deployed (Dashboard)
- **Mô tả chi tiết:** Nếu một tài khoản có cờ `isOnboarded: true` nhưng mảng `shops` trống rỗng (`[]`), khi đăng nhập thành công và bị điều hướng về `/` (Dashboard), ứng dụng lập tức gọi các API Dashboard (như `/api/sales-orders/summary`) nhưng không có `x-shop-id` (vì không có cửa hàng). Điều này gây ra vòng lặp lỗi `400 Bad Request`, kéo theo Flutter UI crash (Silent Failure).
- **Kết quả kỳ vọng:** Khi điều hướng Dashboard, nếu `shops` rỗng, hệ thống phải phát hiện và đẩy người dùng trở lại quy trình Tạo cửa hàng (Onboarding), HOẶC backend chặn 400 và Frontend có trang "Lỗi - Không tìm thấy cửa hàng" thay vì crash.
- **Trạng thái:** Mới ghi nhận (QA Agent bắt được bằng account test).

#### 23. [BUG - A11Y/AUTOMATION] Cây Semantics của Flutter Web bị đóng băng sau khi điều hướng (Routing Freeze)
- **Mức độ:** P0 (Blocker cho QA Automation & Trình Đọc Màn Hình)
- **Môi trường:** Deployed (Toàn hệ thống - Flutter Web)
- **Mô tả chi tiết:** Khi người dùng điều hướng từ `/login` sang `/` (Dashboard) bằng GoRouter, mặc dù Canvas vẽ giao diện Dashboard lên màn hình một cách trực quan, nhưng cây DOM Semantics (`flt-semantics-host`) bị đóng băng (stuck). Nó giữ nguyên các thẻ DOM của màn hình Login. Việc này khiến Automation Bots (Selenium, Playwright) và các trình đọc màn hình của người khiếm thị **không thể click hay tương tác** vào bất kỳ thứ gì trên Dashboard, POS, hay Products.
- **Kết quả kỳ vọng:** Cấu hình lại cơ chế render Web của Flutter (ví dụ dùng `canvaskit` thay vì `auto` hoặc kiểm tra lại việc bọc `Semantics` widget ở root) để cây HTML DOM đồng bộ liên tục với cây Widget mỗi khi chuyển trang.
- **Trạng thái:** Mới ghi nhận (Blocker nghiêm trọng).
