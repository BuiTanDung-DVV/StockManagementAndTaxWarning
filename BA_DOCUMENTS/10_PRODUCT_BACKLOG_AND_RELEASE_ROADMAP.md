...
| P0-17 | Sổ lương gắn nhãn tháng hiện tại nhưng lấy giao dịch ngoài kỳ | Provider gọi trang 1 toàn bộ giao dịch chi với `from/to = null`, sau đó lọc `SALARY` ở frontend | Truyền kỳ vào API, lọc server-side và dùng cùng kỳ cho header/tổng/danh sách/export | Không báo sai chi phí lương theo tháng | P0 | S | Có thể làm thay đổi tổng đang nhìn thấy | Giao dịch 10/07 không nằm trong tháng 8; test ranh giới tháng/timezone đạt |
| P0-18 | Chứng từ mua không hóa đơn có dòng quantity bằng 0 | Seed/import và API chưa khóa invariant số lượng dương | Validate `quantity > 0` ở import, DTO/service và DB constraint sau khi audit dữ liệu; chặn duyệt chứng từ lỗi | Tránh sai tổng mua, giá vốn và chứng từ thuế | P0 | M | Migration fail khi còn dữ liệu lỗi; cần phương án sửa/backfill được duyệt | Validator production = 0 dòng quantity <= 0; UI chỉ cho duyệt khi mọi dòng hợp lệ |
| P0-19 | List/detail cùng mã đơn bán trả khác tên khách | API `findOne` không join quan hệ `customer`, frontend rơi về “Khách mua lẻ” | Join customer trong detail, chuẩn hóa DTO và contract test list/detail/invoice | Không in phiếu, thu nợ hoặc chăm sóc nhầm khách | P0 | S | Có thể lộ dữ liệu đơn cũ thiếu customer thật | `SOY109500` và fixture khác có cùng customer id/name ở list, detail, invoice |
| P0-20 | Deep-link detail dựng `PO-null`/`-0 đ` và vẫn hiện action | Route dùng `state.extra` với fallback object rỗng | Route `:id`, fetch API, not-found/error state; chỉ render action khi entity hợp lệ | Tránh thao tác trên dữ liệu giả và deep-link hỏng | P0 | M | Cần đổi route/link và giữ tương thích tạm | Reload/share detail giữ dữ liệu; id thiếu/sai trả not-found; không có action sửa/xóa |
| P0-21 | Sổ chi phí báo tổng tháng 0 nhưng vẫn liệt kê giao dịch tháng trước | KPI và danh sách không dùng chung kỳ `from/to` | Tạo một filter contract phía server cho summary/chart/list/export; gắn rõ kỳ đang xem | Không đọc sai chi phí và không xuất sai tập dữ liệu | P0 | S | Số hiển thị sẽ thay đổi so với production hiện tại | Với tháng 8 không có dữ liệu: tổng/chart/list cùng rỗng; đổi sang tháng 7 thì tổng bằng tổng dòng |
| P0-22 | Chốt ca coi ô tiền thực tế trống là 0, tạo chênh lệch âm toàn bộ số dư | Giá trị chưa nhập bị ép thành số 0 và công thức chạy ngay | Dùng trạng thái nullable; chỉ tính sau input hợp lệ; xác nhận số thực tế/chênh lệch trước khóa | Tránh ghi nhận và giải trình sai số tiền lớn | P0 | S | Thay đổi validation của form khóa sổ | Ô trống hiển thị “Chưa đối soát”; không tạo chênh lệch; khóa sổ chỉ bật sau số hợp lệ và xác nhận |
| P0-23 | 1.704 đơn lịch sử ghi sai kênh tiền vào 111/112 | Bút toán bán/thu nợ luôn dùng 111 dù thanh toán chuyển khoản/QR/thẻ | Mã mới chọn 111/112 theo phương thức; backup và backfill journal lịch sử bằng payment thực tế, chạy validator trước/sau | Sổ tiền mặt và tiền gửi phản ánh đúng kênh | P0 | M | Backfill sai có thể làm lệch sổ kế toán; cần transaction, snapshot và rollback | Shop 34/35 đều có 0 vi phạm payment–111/112; tổng Nợ/Có vẫn cân; 511/632, cash transaction và payment không đổi |
| P0-24 | Sửa/xóa giao dịch tiền có thể làm lệch journal | API cũ cập nhật/xóa `cash_transactions` nhưng không thay đổi bút toán tương ứng | Chỉ cho sửa giao dịch độc lập; validate input; void và ghi lại journal trong cùng transaction; giao dịch nguồn phải sửa từ chứng từ gốc | Cash flow và sổ kép không tách rời | P0 | S | Người dùng không còn sửa trực tiếp dòng sinh từ đơn hàng | Sửa tiền/phương thức đổi đúng 111/112 và vẫn cân Nợ/Có; lỗi rollback toàn bộ; dòng liên kết trả 400 |
| P0-25 | Owner đăng nhập nhưng header/menu có thể không nhận đúng cửa hàng | Login shop context thiếu tên/mã; frontend chỉ dùng mảng login và không hydrate ngay | Backend trả đầy đủ định danh shop; frontend tải lại `/my-shops` sau login và giữ shop hợp lệ | Menu quyền và hành động đúng ngay lần vào đầu | P0 | S | Thêm một request sau login; cần tránh nhấp nháy loading | Owner thấy đúng tên shop, đủ menu và quyền thêm; reload vẫn giữ shop; đổi tất cả ↔ một shop hoạt động |
| P0-26 | Bảng chậm luân chuyển hiện “Sản phẩm chưa có tên” và có thể đánh giá sai lần bán cuối | Response trả `name/currentStock` nhưng UI đọc field khác; tiêu chí cũ dựa trên mọi phiếu `OUT` | Chuẩn hóa response; lấy lần bán cuối từ đơn bán không hủy; trả số ngày chưa bán và đơn vị | Cảnh báo nhập/xả hàng dựa đúng hành vi bán | P0 | S | Seed cũ có thể lộ mặt hàng chưa từng có đơn bán thật | Tên/tồn/đơn vị đầy đủ; sản phẩm bán trong 30 ngày không xuất hiện; ngày bán cuối khớp query đơn hàng |
| P0-27 | Báo cáo bán hàng và tài chính có thể lệch ngày ở giao dịch gần 0 giờ | Nhiều service dùng `new Date(YYYY-MM-DD)` theo múi giờ máy chủ, khác bộ lọc giao dịch theo giờ Việt Nam | Dùng chung bộ phân tích kỳ Việt Nam cho summary, top sản phẩm, thanh toán, P&L và hóa đơn | Các màn hình đối soát cùng một tập giao dịch | P0 | S | Cần kiểm tra kiểu `timestamp` lịch sử trước migration | Fixture 00:00 và 23:59 giờ Việt Nam nằm đúng kỳ; dashboard/sales/finance/invoice khớp tổng kiểm soát |
| P0-28 | AI báo mọi sản phẩm có định mức là sắp hết và đọc nợ từ số dư cache | Query insight không so tồn thực tế; công nợ dùng `customers.balance` thay vì khoản phải thu | Đếm tồn theo tổng `inventory_stocks`; tổng nợ từ receivable còn mở, loại paid/cancelled | Cảnh báo AI phản ánh dữ liệu nghiệp vụ thật | P0 | S | Query tổng hợp cần index theo shop/product/status | Insight tồn khớp danh sách dưới định mức; tổng nợ AI khớp báo cáo tuổi nợ |
| P0-29 | Chốt ca có thể không thấy giao dịch và chấp nhận tổng tiền do client tự gửi | Tìm transaction bằng timestamp tuyệt đối; `orderCount` thực chất là số giao dịch; backend lưu thẳng DTO | Dùng biên ngày Việt Nam; lấy đơn/doanh số/trả hàng từ DB; server tự tính toàn bộ số kiểm soát | Khóa sổ đúng ngày và chống sửa số từ client | P0 | M | Cần smoke test ca có chuyển khoản, trả hàng và lệch két | Danh sách ngày đủ giao dịch; đơn loại cancelled; sửa totals trong request không làm đổi số lưu; chênh lệch tính từ tiền thực tế |
| P0-30 | Cột biểu đồ có thể lệch một ngày dù tổng kỳ đúng | SQL `TO_CHAR(timestamp)` và vòng lặp nhãn dùng múi giờ server | Gom nhóm SQL theo `Asia/Ho_Chi_Minh`; sinh nhãn ngày/tháng bằng lịch Việt Nam | Cột ngày khớp chứng từ và trục X | P0 | S | Cần kiểm tra kiểu cột timestamp/timestamptz trên production | Giao dịch 00:00 và 23:59 giờ Việt Nam nằm đúng cột; tổng các cột bằng KPI cùng kỳ |
| P0-31 | Bảng kê mua không hóa đơn có thể ở trạng thái approved nhưng chưa vào kho hoặc bị ghi lặp | Chủ shop được duyệt tự động nhưng side effect chỉ chạy ở endpoint duyệt; mất `warehouseId`; không khóa quyết định | Transaction chung cho lưu/duyệt/kho/lot/cash/journal; mặc định kho hợp lệ; quyết định idempotent và bất biến | Tồn kho, dòng tiền và sổ cái không phân kỳ | P0 | M | Bản approved lịch sử cần validator/backfill riêng | Owner tạo approved sinh đúng một movement/lot/cash/journal; gọi duyệt lại không đổi tồn; chuyển khoản ghi 112 |
| P0-32 | Lợi nhuận và hoàn hàng sai khi đơn có thuế hoặc còn nợ | 511 ghi cả VAT; hoàn chỉ đảo số tiền đã trả, tin đơn giá client và bỏ dư 131 | Tách 511/3331; return full dùng dữ liệu order; hoàn theo payment ledger và đảo 131/632/156; báo cáo trừ toàn đơn trả | P&L, thuế, công nợ và tồn kho khớp nhau | P0 | M | Bút toán lịch sử cần migration có backup; hiện chỉ sửa giao dịch mới | Đơn có VAT cân Nợ/Có; trả đơn trả góp đưa 511/3331/131/632/156 về 0; tổng báo cáo giảm đúng toàn giá trị trả |
| P0-33 | Top sản phẩm phóng đại doanh thu/lãi khi đơn có chiết khấu | Dòng hàng giữ giá trước giảm nhưng truy vấn xếp hạng không phân bổ `discount_amount` của đơn | Phân bổ giảm giá theo tỷ trọng thành tiền cho kỳ hiện tại, kỳ trước và hàng trả; loại VAT khỏi doanh thu hàng hóa | Bảng sản phẩm, tăng trưởng và biên lãi đối soát được với doanh thu thuần | P0 | S | Đơn lịch sử có subtotal 0 hoặc discount vượt subtotal cần chặn an toàn | Tổng phân bổ các dòng bằng subtotal trừ giảm giá; đơn 1 triệu giảm 100 nghìn phân bổ dòng 400 nghìn còn 360 nghìn; truy vấn chạy được trên shop 34/35 |
...
| V11-13 | Bảng XNT mobile bị cắt, biểu đồ ngưỡng thuế vỡ chữ | Component desktop co trực tiếp vào 390 px | XNT dùng record card/column priority và chỉ dấu cuộn; ngưỡng thuế dùng threshold card ở mobile | Báo cáo đọc được không cần phóng to hoặc đoán cột | V1.1 | M | Hai chế độ hiển thị phải dùng cùng dữ liệu | 390×844 đọc đủ tồn đầu/nhập/xuất/tồn cuối; mốc 1 tỷ và trạng thái hiện tại không vỡ chữ |
| V11-14 | Kế hoạch ngân sách hiển thị số thực tế có thể nhập tay | API lưu `actualIncome/actualExpense` từ client; UI thiếu luồng tạo kế hoạch và mốc ngày | Tính số thực tế từ giao dịch DB theo khoảng ngày; chỉ lưu mục tiêu; bổ sung tạo kế hoạch và hiển thị kỳ | Theo dõi ngân sách đúng dữ liệu vận hành | V1.1 | M | Query tổng hợp nhiều kế hoạch cần theo dõi hiệu năng | Client không sửa được số thực tế; tổng thu/chi khớp giao dịch trong kỳ; card không tràn desktop/mobile |
| V11-15 | KPI kỳ hiện tại có thể bằng 0 khi dữ liệu đã dừng ở kỳ trước | UI ghi ngày hiện tại nhưng API không trả ngày phát sinh mới nhất | Trả ngày MAX từ DB theo scope; cảnh báo trên Dashboard/Tài chính/Kho; thêm data-freshness audit gate | Không nhầm “chưa có dữ liệu” với kết quả bằng 0 | V1.1 | S | Cảnh báo quá nhiều ở cửa hàng ít phát sinh | Ngày gần nhất khớp DB; kỳ thiếu dữ liệu có cảnh báo; kỳ đủ dữ liệu không chiếm chỗ UI; mobile không tràn |
