...
const START_DATE = new Date('2022-01-01T08:00:00+07:00');
const END_DATE = new Date('2026-12-31T17:30:00+07:00');
const DATA_SCALE = 5;
const PURCHASE_CYCLES_PER_MONTH = 5;
const TOTAL_MONTHS = 60;
const STOCK_TAKE_COUNT = 100;
...
function expandStoreProfile(base: StoreProfile): StoreProfile {
  ...
  return {
    ...base,
    datasetVersion: `${base.datasetVersion}_5Y_V1`,
...
const STORE_PROFILES: Record<ProfileKey, StoreProfile> = {
  construction: {
    key: 'construction',
    datasetVersion: 'STORE_HISTORY_5Y_CONSTRUCTION',
    skuPrefix: 'VL',
    description: 'Vật liệu xây dựng, điện nước, đồ gia dụng và thiết bị phòng tắm',
    peakMonths: [0, 1, 9, 10, 11], // Tháng 1, 2, 10, 11, 12 (mùa khô & sửa nhà Tết)
    openingCash: 2500000000,
    openingBank: 500000000,
    products: [
  ...
  agriculture: {
    key: 'agriculture',
    datasetVersion: 'STORE_HISTORY_5Y_AGRICULTURE',
    skuPrefix: 'NN',
    description: 'Phân bón, hạt giống, bảo vệ thực vật và vật tư nông nghiệp',
    peakMonths: [0, 1, 3, 4, 7, 8], // Vụ Đông Xuân & Hè Thu
    openingCash: 5000000000,
    openingBank: 1200000000,
    products: [
...
async function clearExistingShopData(
  ...
  const statements = [
    'UPDATE sales_orders SET shipping_expense_transaction_id = NULL WHERE shop_id = $1',
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    const match = statement.match(/(?:FROM|UPDATE)\s+([a-z_]+)/i);
    const table = match ? match[1] : `statement ${i}`;
    process.stdout.write(`[${i + 1}/${statements.length}] Tạm dọn dẹp ${table}... `);
...
async function validateGeneratedData(
  ...
  const failures = [
    ...
    ['purchaseCount', Math.abs(Number(result.purchaseCount) - TOTAL_MONTHS * PURCHASE_CYCLES_PER_MONTH)],
    ...
    ['purchaseWithoutInvoiceCount', Number(result.purchaseWithoutInvoiceCount) >= 12 ? 0 : 1],
  ...
  const metricRows = await runner.query(`
    ...
    SELECT
      ...
      sales.year_4::numeric AS "year4Revenue",
      sales.year_5::numeric AS "year5Revenue"
  ...
  const year4 = Number(metrics.year4Revenue);
  const year5 = Number(metrics.year5Revenue);
  ...
  const qualitySummary = {
    ...
    year4Revenue: Math.round(year4),
    year5Revenue: Math.round(year5),
  };
  ...
  const qualityFailures = [
    ...
    year4 <= year3 * 1.02 ? 'doanh thu năm 4 không tăng tối thiểu 2%' : '',
    year5 <= year4 * 1.02 ? 'doanh thu năm 5 không tăng tối thiểu 2%' : '',
  ].filter(Boolean);
  if (qualityFailures.length > 0) {
    throw new Error(`Chất lượng số liệu 5 năm chưa đạt: ${qualityFailures.join('; ')}`);
  }

  console.log('Đối soát nội bộ 5 năm: PASS (toàn vẹn dữ liệu kế toán, thẻ kho và cảnh báo thuế).');
}
...
function printPlan(shopId?: number, profile?: StoreProfile): void {
  ...
  console.log('========================================================');
  console.log('Kế hoạch bộ dữ liệu vận hành 5 năm (2022 - 2026)');
  console.log(`- Phiên bản: ${profile?.datasetVersion ?? '(chưa chọn hồ sơ ngành)'}`);
  console.log(`- Cửa hàng ID: ${shopId ?? '(chưa chỉ định)'}`);
  console.log(`- Hồ sơ ngành: ${profile?.description ?? '(chưa chỉ định)'}`);
  console.log(`- Thời gian: ${dateOnly(START_DATE)} đến ${dateOnly(END_DATE)} (${days} ngày / ${TOTAL_MONTHS} tháng)`);
  console.log(
    `- Quy mô: ${profile?.products.length ?? 0} sản phẩm, ` +
    `${profile?.customers.length ?? 0} khách hàng, ${profile?.suppliers.length ?? 0} nhà cung cấp`,
  );
  console.log(
    `- Khoảng 45.000–55.000 đơn bán, ${TOTAL_MONTHS * PURCHASE_CYCLES_PER_MONTH} kỳ nhập hàng, ` +
    `20 quý nghĩa vụ thuế, và ${STOCK_TAKE_COUNT} lần kiểm kê`,
  );
  console.log('- Mặc định không ghi database. Dùng --validate-write để dry-run hoặc --apply để ghi thật.');
  console.log('========================================================');
}
...
async function seed(
  ...
  await runner.query("SET LOCAL statement_timeout = '20min'");
  ...
  try {
    ...
    // Tận dụng 100% tài nguyên ảnh sản phẩm hiện có
    ...
    // Tận dụng 100% chứng từ, hóa đơn scan và avatar hiện có
    ...
    if (owners.length < 1) {
      throw new Error(`Cửa hàng phải có ít nhất 1 owner đang hoạt động, hiện có ${owners.length}`);
    }
    const ownerId = Number(owners[0].user_id);

    if (replaceExisting || !commitChanges) {
      console.log(commitChanges ? `Đang xóa dữ liệu cũ của cửa hàng ${shopId}...` : `[DRY-RUN] Tạm xóa dữ liệu cũ trong transaction để đối soát...`);
    ...
    await bulkInsert(
      ...
      operationalTagNames.map((name, index) => ({
        ...
        name,
        color: [
          '#2563EB', '#0F766E', '#7C3AED', '#C2410C', '#0369A1',
          '#4D7C0F', '#DC2626', '#D97706', '#0891B2',
        ][index % 9],
        type: 'product',
      })),
    );

    // 3. Sản phẩm (Products) - Tái sử dụng trọn vẹn URL Cloudinary
    ...
    const productRows = profile.products.map((definition, index) => ({
      ...
      cost_price: roundMoney(definition.cost * 1.15),
      selling_price: roundMoney(definition.sell * 1.20),
      wholesale_price: roundMoney(definition.sell * 1.20 * 0.94),
    ...
    // 4. Loại chi phí (Cost Types) & Giá vốn định mức (Cost items) & Lịch sử giá (Price history)
    ...
    for (let index = 0; index < products.length; index += 1) {
      ...
      const baseSell = definition.sell;
      priceHistoryRows.push(
        {
          product_id: productId,
          shop_id: shopId,
          price_type: 'SELLING',
          old_price: roundMoney(baseSell),
          new_price: roundMoney(baseSell * 1.05),
          change_reason: 'Cập nhật giá bán năm 2023',
          changed_by: ownerId,
          changed_at: new Date('2023-01-05T08:00:00+07:00'),
        },
        {
          product_id: productId,
          shop_id: shopId,
          price_type: 'SELLING',
          old_price: roundMoney(baseSell * 1.05),
          new_price: roundMoney(baseSell * 1.10),
          change_reason: 'Cập nhật giá bán năm 2025',
          changed_by: ownerId,
        ...
        },
        {
          ...
          shop_id: shopId,
          price_type: 'SELLING',
          old_price: roundMoney(baseSell * 1.10),
          new_price: roundMoney(baseSell * 1.20),
          change_reason: 'Cập nhật giá bán kế hoạch 2026-2027',
          changed_by: ownerId,
      ...
      if (conversion) {
        conversionRows.push({
          ...
          selling_price_per_unit: roundMoney((definition.sell * 1.20) / conversion.rate),
        });
      }
    }
    await bulkInsert(
    ...
    const customers = await bulkInsert(
      ...
      ['code', 'shop_id', 'name', 'phone', 'address', 'avatar_url', 'identity_image_url',
        'customer_type', 'credit_limit', 'balance', 'notes', 'is_active', 'created_at', 'updated_at'],
      customerRows,
      'id, code',
    );
    ...
    // 7. Kho hàng (Warehouse) & Tài khoản tiền (Cash Accounts)
    ...
    // 8. Mô phỏng sinh đơn hàng 6 năm (2022 - 2027)
    console.log('Đang mô phỏng chuỗi bán hàng 6 năm...');
    ...
    for (let dayIndex = 0; dayIndex < days; dayIndex += 1) {
      ...
      const operatingYear = Math.min(Math.floor(dayIndex / 365), 4); // 0 (2022) đến 4 (2026)
      
      // Số đơn/ngày tăng trưởng theo các năm:
      // Năm 2022: ~16-20 đơn/ngày
      // Năm 2027: ~31-35 đơn/ngày
      const orderCount =
        15 +
        operatingYear * 3 +
        (weekend ? 3 : 0) +
      ...
      for (let orderIndex = 0; orderIndex < orderCount; orderIndex += 1) {
        ...
        while (items.length < itemCount) {
          ...
          const unitPrice = roundMoney(
            definition.sell * (1 + operatingYear * 0.04),
          );
          const costPrice = roundMoney(
            definition.cost * (1 + operatingYear * 0.03),
          );
        ...
        const discount = subtotal >= 2000000 && random() < 0.20
          ? roundMoney(subtotal * (0.02 + random() * 0.03))
          : 0;
        const total = subtotal - discount;
        const cancelled = random() < 0.015;
        ...
        const debtPaid = unpaid > 0 && canCollect && random() < 0.72 ? unpaid : 0;
        ...
        const method =
          methodRoll < 0.22
            ...
            : methodRoll < 0.74
              ? 'TRANSFER'
              : 'QR';
        const debtMethod = random() < 0.10 ? 'CASH' : 'TRANSFER';
    ...
    console.log(`Đã tạo cấu trúc ${generatedOrders.length} đơn hàng. Đang lưu sales_orders...`);
    ...
    // Vốn đầu kỳ (01/01/2022)
    cashRows.push({
      transaction_code: `T6${key}${String(sequence++).padStart(7, '0')}`,
      shop_id: shopId,
      type: 'INCOME',
      category: 'CAPITAL',
      ...
      notes: 'Số dư tiền mặt đầu kỳ 2022',
      created_by: ownerId,
      created_at: START_DATE,
    });
    cashRows.push({
      ...
      reference_id: null,
      transaction_date: dateOnly(START_DATE),
      notes: 'Số dư tài khoản ngân hàng đầu kỳ 2022',
      created_by: ownerId,
      created_at: START_DATE,
    });
    ...
    for (const order of generatedOrders) {
      ...
      if (order.initialPaid > 0) {
        ...
        cashRows.push({
          ...
          notes: `Thu tiền bán hàng ${order.code}`,
          created_by: ownerId,
          created_at: order.date,
        });
      }
      ...
      if (!order.cancelled && originalDebt > 0) {
        ...
        receivableRows.push({
          ...
          notes: `Công nợ từ đơn ${order.code}`,
          debt_reason: 'Khách mua hàng gối đầu theo công trình/mùa vụ',
        ...
        if (order.debtPaid > 0) {
          ...
          paymentRows.push({
            ...
            method: order.debtMethod,
            reference_code: `${order.code}-D1`,
            notes: 'Thu công nợ gối đầu',
            paid_at: paidAt,
          });
          cashRows.push({
            transaction_code: `T6${key}${String(sequence++).padStart(7, '0')}`,
            ...
            amount: order.debtPaid,
            payment_method: order.debtMethod,
            account_id: order.debtMethod === 'CASH' ? cashAccountId : bankAccountId,
            counterparty: profile.customers[order.customerIndex],
            reference_type: 'SALES_ORDER',
            reference_id: orderId,
            transaction_date: dateOnly(paidAt),
            notes: `Thu hồi nợ ${order.code}`,
            created_by: ownerId,
            created_at: paidAt,
          });
          journalRows.push({
            ...
            reference_id: orderId,
            description: `Thu công nợ khách hàng ${order.code}`,
            is_voided: false,
            created_at: paidAt,
          });
        }
      ...
      if (!order.cancelled && random() < 0.33) {
        invoiceRows.push({
          ...
          image_url: invoiceImageUrls.length
            ...
            : null,
          notes: 'Hóa đơn điện tử GTGT theo Thông tư 78/2021',
          created_by: ownerId,
          created_at: order.date,
        });
      }
    ...
    console.log(`Lưu ${selectedReturns.length} phiếu đổi trả...`);
    const insertedReturns = await bulkInsert(
      ...
      selectedReturns.map((salesReturn) => ({
        ...
        reason: 'Khách thừa vật tư công trình hoàn lại',
        refund_amount: salesReturn.refund,
        refund_method: salesReturn.method,
        status: 'COMPLETED',
        processed_by: ownerId,
        notes: 'Hàng nguyên vẹn đã nhập lại thẻ kho',
    ...
    for (const insertedReturn of insertedReturns) {
      ...
      for (const item of salesReturn.items) {
        ...
        movementRows.push({
          ...
          notes: `Nhập kho hàng trả ${salesReturn.code}`,
          created_by: ownerId,
          created_at: salesReturn.date,
        });
      }
      cashRows.push({
        ...
        counterparty: 'Khách hoàn hàng',
        reference_type: 'SALES_RETURN',
        reference_id: Number(insertedReturn.id),
        transaction_date: dateOnly(salesReturn.date),
        notes: `Chi hoàn tiền ${salesReturn.code}`,
        created_by: ownerId,
        created_at: salesReturn.date,
      });
    }
    ...
    console.log('Lưu sales_order_items & payments & receivables & movements & invoices...');
    await bulkInsert(
    ...
    const insertedInvoices = await bulkInsert(
      ...
      ['invoice_number', 'shop_id', 'invoice_symbol', 'invoice_type', 'invoice_date',
        ...
        'created_at'],
      invoiceRows, 'id, reference_id', 500,
    );

    // Gắn bằng chứng scan hóa đơn & biên nhận thu nợ
      ...
      await bulkInsert(
        ...
        insertedInvoices
          ...
          .map((invoice, index) => ({
            ...
            ocr_parsed_data: JSON.stringify({ source: 'verified-test-dataset', verified: true }),
            ...
            notes: 'Ảnh hóa đơn đối chiếu thực tế',
          })),
        '',
        300,
      );
      ...
      await bulkInsert(
        ...
        insertedReceivables
          ...
          .map((receivable, index) => ({
            ...
            file_size: null,
            description: 'Biên bản đối chiếu và giấy nhận nợ khách hàng',
            uploaded_by: ownerId,
    ...
    const debtHistoryRows = insertedReceivables
      ...
      .map((receivable) => {
        const order = orderById.get(Number(receivable.order_id));
        const method = order?.debtMethod || 'TRANSFER';
        return {
          receivable_id: Number(receivable.id),
          shop_id: shopId,
          ...
          notes: method === 'CASH' ? 'Thu nợ tiền mặt' : 'Thu nợ chuyển khoản ngân hàng',
          recorded_by: ownerId,
    ...
    // 9. Nhập hàng 6 năm (360 kỳ nhập)
    console.log('Đang mô phỏng và lưu chuỗi mua hàng (purchase_orders)...');
    ...
    for (const [keyMonth, soldQuantities] of [...monthlySold.entries()].sort()) {
      for (
        ...
        const code = `P6${key}${String(purchaseNumber).padStart(4, '0')}`;
        ...
        const items = soldQuantities.flatMap((sold, index) => {
          ...
          return [{
            ...
            quantity:
              ...
              (purchaseNumber === 1
                ? profile.products[index].minStock * 4
                : 0),
            unit_price: roundMoney(
              profile.products[index].cost * (1 + purchaseYear * 0.03),
            ),
          }];
        });
        const total = items.reduce((sum, item) => sum + item.quantity * item.unit_price, 0);
        const initialPaid = roundMoney(total * 0.85);
        const outstanding = total - initialPaid;
        const debtPaymentDate = addDays(purchaseDate, 25);
        const debtPaid =
          debtPaymentDate < END_DATE && random() < 0.94 ? outstanding : 0;
        ...
        const finalPaid = initialPaid + debtPaid;
        const purchasePaymentMethod = random() < 0.17 ? 'CASH' : 'TRANSFER';
        ...
        purchaseRows.push({
          ...
          notes: 'Nhập hàng định kỳ từ nhà phân phối',
          created_by: ownerId,
        ...
        initialPaymentSchedule.forEach((payment, scheduleIdx) => {
          cashRows.push({
            transaction_code: `T6${key}${String(sequence++).padStart(7, '0')}`,
            shop_id: shopId,
            type: 'EXPENSE',
            ...
            account_id: purchasePaymentMethod === 'CASH' ? cashAccountId : bankAccountId,
            ...
            notes: initialPaymentSchedule.length === 1
              ...
              : `Thanh toán đợt ${scheduleIdx + 1}/${initialPaymentSchedule.length} nhập hàng ${code}`,
            created_by: ownerId,
            created_at: payment.date,
          });
        });
        debtPaymentSchedule.forEach((payment, scheduleIdx) => {
          cashRows.push({
            transaction_code: `T6${key}${String(sequence++).padStart(7, '0')}`,
            shop_id: shopId,
            type: 'EXPENSE',
            ...
            notes: debtPaymentSchedule.length === 1
              ...
              : `Thanh toán công nợ đợt ${scheduleIdx + 1}/${debtPaymentSchedule.length} nhập hàng ${code}`,
            created_by: ownerId,
            created_at: payment.date,
          });
        });
    ...
    const insertedPurchases = await bulkInsert(
      ...
      purchaseRows, 'id, order_code, order_date, supplier_id, total_amount, paid_amount', 300,
    );
    ...
    for (const purchase of insertedPurchases) {
      ...
      if (purchaseWithoutInvoiceCodes.has(code)) {
        ...
        purchaseWithoutInvoiceRows.push({
          ...
          notes: `Bảng kê chứng từ mua hàng không hóa đơn ${code}`,
        ...
        purchaseInvoiceRows.push({
          ...
          notes: 'Hóa đơn mua hàng đầu vào',
          created_by: ownerId,
          created_at: purchase.order_date as Date,
        });
      }
    ...
    const payableHistoryRows = insertedPayables.flatMap((payable) => {
      ...
      return schedule.map((payment, scheduleIdx) => ({
          ...
          notes: schedule.length === 1
            ...
            : `Thanh toán công nợ nhà cung cấp đợt ${scheduleIdx + 1}/${schedule.length}`,
          recorded_by: ownerId,
          created_at: payment.date,
        }));
    });
    ...
    // 10. Chi phí hoạt động & Lệ phí Môn bài từng năm
    console.log('Đang hạch toán chi phí vận hành & lệ phí môn bài...');
    ...
    for (let cursor = new Date(START_DATE); cursor <= END_DATE; cursor = addDays(cursor, 1)) {
      ...
      // Lệ phí môn bài vào ngày 15 tháng 1 hàng năm
      if (cursor.getMonth() === 0 && cursor.getDate() === 15) {
        expenseRows.push({
          date: atLocalTime(cursor, 9, 30),
          category: 'TAX',
          amount: 1000000,
          note: `Lệ phí môn bài năm ${currentYear}`,
        });
      }

      if (cursor.getDate() === 5) {
        const rentAmount = 16000000 + (currentYear - 2022) * 1500000;
        expenseRows.push({ date: atLocalTime(cursor, 9, 0), category: 'RENT', amount: rentAmount, note: 'Thuê mặt bằng kinh doanh' });
      }
      if (cursor.getDate() === 10) {
        const salaryAmount = 36000000 + (currentYear - 2022) * 3000000;
        expenseRows.push({ date: atLocalTime(cursor, 10, 0), category: 'SALARY', amount: salaryAmount, note: 'Lương nhân viên cửa hàng' });
      }
      if (cursor.getDate() === 18) {
        expenseRows.push({ date: atLocalTime(cursor, 15, 0), category: 'UTILITIES', amount: 4500000 + Math.floor(random() * 2000000), note: 'Điện nước và viễn thông' });
      }
      if (dayIndex % 3 === 0) {
        expenseRows.push({ date: atLocalTime(cursor, 16, 30), category: 'DELIVERY', amount: 180000 + Math.floor(random() * 520000), note: 'Giao nhận và bốc xếp hàng hóa' });
      }
    }
    ...
    // 11. Bút toán sổ cái kép (Journal Entries & Lines)
    console.log('Đang lập sổ cái kép cân đối tài khoản 111, 112, 131, 156, 331, 333, 511, 632, 642...');
    ...
    for (const journal of insertedExpenseJournals) {
      ...
      expenseJournalLineRows.push(
        {
          journal_entry_id: Number(journal.id),
          account_code: expense.category === 'TAX' ? '333' : '642',
          amount,
          entry_type: 'DEBIT',
          created_at: expense.date,
        },
    ...
    // 12. Tồn kho và Thẻ kho (Inventory Stocks & Adjustments)
    console.log('Đang đồng bộ thẻ kho và cập nhật số dư tồn kho...');
    await runner.query(`
    ...
    for (let index = 0; index < stockBeforeAdjustments.length; index += 1) {
      ...
      // Tạo một số mặt hàng chạm ngưỡng tồn thấp để test cảnh báo
      ...
      stockAdjustmentRows.push({
        ...
        reference_id: null,
        notes: index % 31 === 0
          ? 'Điều chỉnh thanh lý hàng trưng bày cuối kỳ'
          : 'Điều chỉnh tồn thấp để kích hoạt cảnh báo nhập hàng',
        created_by: ownerId,
        created_at: END_DATE,
      });
    }
    ...
    // 13. Lô hàng và Hạn sử dụng (Product Batches & Inventory Lots)
    ...
    for (let index = 0; index < stockSnapshot.length; index += 1) {
      ...
      batchRows.push({
        ...
        notes: tracksExpiry
          ? 'Lô hàng đang theo dõi hạn sử dụng'
          : 'Lô hàng đang lưu kho bình thường',
        is_active: true,
    ...
    // 14. Kiểm kê định kỳ (120 đợt kiểm kê qua 6 năm)
    console.log(`Đang lập ${STOCK_TAKE_COUNT} phiếu kiểm kê định kỳ...`);
    ...
    for (let index = 0; index < STOCK_TAKE_COUNT; index += 1) {
      ...
      stockTakeRows.push({
        ...
        status: 'COMPLETED',
        notes: 'Kiểm kê kho định kỳ, đối chiếu thẻ kho với thực tế',
        created_by: ownerId,
        approved_by: ownerId,
    ...
    // 15. Cập nhật số dư công nợ khách hàng, nhà cung cấp và quỹ tiền
    await runner.query(`
    ...
    // 16. Chốt quỹ hàng ngày (Daily Closings) - 2.191 ngày liên tục
    console.log(`Đang lập và chốt quỹ ${days} ngày hoạt động liên tục (2022 - 2027)...`);
    ...
    for (const day of dailyRows) {
      ...
      closingRows.push({
        ...
        notes: 'Chốt quỹ và bàn giao ca cuối ngày',
        closed_by: ownerId,
    ...
    // 17. Dự báo dòng tiền (Cashflow Forecasts) & Kế hoạch ngân sách 72 tháng (Budget Plans)
    console.log('Đang lập dự báo dòng tiền và kế hoạch ngân sách...');
    ...
    for (let index = 1; index <= 90; index += 1) {
      ...
      forecastRows.push({
        ...
        notes: 'Dự báo dòng tiền vận hành quý tiếp theo',
        created_at: END_DATE,
      });
    }
    await bulkInsert(
    ...
    // 18. Nghĩa vụ thuế 24 quý (2022 - 2027)
    console.log('Đang lập 24 quý nghĩa vụ thuế GTGT & TNCN (Thông tư 40/2021)...');
    ...
    const currentSimDate = new Date('2026-09-01T00:00:00+07:00');
    const taxRows = [...quarterRevenue.entries()].map(([period, revenue]) => {
      ...
      const dueDate = new Date(yearNum, quarterNumber * 3, 30);
      const vat = roundMoney(revenue * 0.01);
      const pit = roundMoney(revenue * 0.005);
      // Các quý trước mốc 2026-09-01 đã hoàn thành nộp thuế
      const isPastAndPaid = dueDate < currentSimDate;
      return {
        period,
        shop_id: shopId,
        vat_declared: vat,
        pit_declared: pit,
        vat_paid: isPastAndPaid ? vat : 0,
        pit_paid: isPastAndPaid ? pit : 0,
        due_date: dateOnly(dueDate),
        status: isPastAndPaid ? 'done' : 'pending',
        created_at: new Date(yearNum, (quarterNumber - 1) * 3, 1),
      };
    });
    await bulkInsert(
    ...
    // 19. Tài liệu tri thức AI (AI Knowledge Documents)
    console.log('Đang ghi nhận tài liệu tri thức AI...');
    await bulkInsert(
      ...
      [
        {
          shop_id: shopId,
          title: 'Quy trình bán chịu và kiểm soát công nợ',
          category: 'Bán hàng & Sổ nợ',
          content: 'Chỉ bán chịu cho khách hàng đã có hồ sơ và hạn mức tín dụng. Ghi nhận số tiền trả trước, ngày đến hạn và số ngày được nợ. Đối chiếu công nợ hàng tuần và đính kèm biên bản đối chiếu nợ.',
          is_active: true,
          created_by: ownerId,
          created_at: START_DATE,
          updated_at: END_DATE,
        ...
        {
          shop_id: shopId,
          title: 'Quy trình nhập kho và quản lý hạn sử dụng',
          category: 'Kho & Tài chính',
          content: 'Đơn nhập hàng phải có hóa đơn hoặc bảng kê chứng từ hợp lệ. Hàng hóa theo lô có hạn sử dụng phải được kiểm tra date trước khi nhập kho. Thực hiện kiểm kê định kỳ và xử lý hàng cận date trước 30 ngày.',
          is_active: true,
          created_by: ownerId,
          created_at: START_DATE,
          updated_at: END_DATE,
        ...
        {
          shop_id: shopId,
          title: 'Quy trình kê khai thuế và chốt quỹ cuối ngày',
          category: 'Kho & Tài chính',
          content: 'Kê khai thuế GTGT và TNCN theo phương pháp kê khai theo quý. Đối chiếu tiền mặt thực tế và tài khoản ngân hàng hàng ngày, đảm bảo số dư không âm và hoàn thành nộp thuế trước ngày 30 tháng đầu quý sau.',
          is_active: true,
          created_by: ownerId,
          created_at: START_DATE,
          updated_at: END_DATE,
    ...
    // 20. Nhật ký hoạt động (Activity Logs)
    console.log('Đang ghi nhận nhật ký hoạt động hệ thống...');
    const activityLogRows: Row[] = monthTotals.map(
      (month: Record<string, unknown>, index: number) => ({
        ...
        description: 'Đã tổng hợp và phê duyệt báo cáo vận hành tháng',
        ip_address: null,
        created_at: addDays(START_DATE, Math.min(index * 30 + 27, 2190)),
      }),
    );
    activityLogRows.push(
      ...
      })),
      {
        ...
        description: 'Khởi tạo bộ dữ liệu lịch sử vận hành 5 năm (2022 - 2026)',
        ip_address: null,
        created_at: END_DATE,
      },
    );
    ...
    console.log('Đang kiểm tra và đối soát các chỉ số chất lượng...');
    await validateGeneratedData(runner, shopId, profile.products.length);

    if (commitChanges) {
      await runner.commitTransaction();
      console.log(`>>> HOÀN TẤT GHI DATABASE CHO CỬA HÀNG "${shops[0].shop_name}" (Shop ID: ${shopId}) <<<`);
    } else {
      await runner.rollbackTransaction();
      console.log(`>>> ĐỐI SOÁT DRY-RUN HOÀN HẢO CHO CỬA HÀNG "${shops[0].shop_name}" (Rollback an toàn) <<<`);
    }
    ...
    console.log(`- 20 quý nghĩa vụ thuế; ${STOCK_TAKE_COUNT} lần kiểm kê`);
  } catch (error) {
    console.error('LỖI GỐC GÂY THẤT BẠI:', error);
    try {
      if (runner.isTransactionActive) {
        await runner.rollbackTransaction();
      }
    } catch (rbErr) {
      console.error('Lỗi phụ khi rollback:', rbErr);
    }
    throw error;
  } finally {
    try {
      ...
      // Bỏ qua lỗi nếu connection runner đã được release trước đó
    }
