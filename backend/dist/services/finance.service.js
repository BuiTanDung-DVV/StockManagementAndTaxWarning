...
class FinanceService {
    constructor() {
        ...
        this.taxObRepo = db_config_1.AppDataSource.getRepository(entities_1.TaxObligation);
        this.purchaseNoInvRepo = db_config_1.AppDataSource.getRepository(entities_1.PurchaseWithoutInvoice);
    }
    async getCashTransactions(page = 1, limit = 20, type, from, to) {
        const qb = this.cashTxRepo.createQueryBuilder('t');
        if (type)
            qb.andWhere('t.type = :type', { type });
        if (from)
            qb.andWhere('t.transaction_date >= :from', { from: new Date(from) });
        if (to)
            qb.andWhere('t.transaction_date <= :to', { to: new Date(to) });
    ...
    async getProfitLoss(from, to) {
        ...
        const salesResult = await db_config_1.AppDataSource.createQueryBuilder()
    ...
    async getDailyClosingByDate(date) {
        const closing = await this.closingRepo.findOne({ where: { closingDate: new Date(date) } });
        if (!closing) {
    ...
    async getInvoices(page = 1, limit = 20, type) {
        const qb = this.invoiceRepo.createQueryBuilder('i');
        if (type)
            qb.andWhere('i.invoice_type = :type', { type });
    ...
    async createInvoice(dto) {
        if (!dto.invoiceNumber)
            dto.invoiceNumber = 'HD' + Date.now().toString().slice(-8);
    ...
    async createPurchaseWithoutInvoice(dto) {
        if (!dto.recordCode)
            dto.recordCode = 'BK-' + Date.now().toString().slice(-6);
